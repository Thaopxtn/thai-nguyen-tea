...
export async function getRelatedNews(currentId, limit = 3) {
    try {
        const conn = await dbConnect();
        if (!conn) throw new Error("No DB Connection");

        // Simple logic: get latest news excluding current one
        // ideally we might want random or same category, but simple is fine for now
        const related = await News.find({ id: { $ne: currentId } })
            .sort({ createdAt: -1 })
            .limit(limit);
        ...
        return MOCK_NEWS.filter(n => n.id !== currentId).slice(0, limit);
    }
}
